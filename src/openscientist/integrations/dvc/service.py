...
def _asset(dataset_dir: Path, path: Path, role: str) -> DVCAsset:
    return DVCAsset(
...
class DVCAcquisitionService:
    ...
    def test_connection(self, connection_id: str) -> dict[str, Any]:
        ...
        try:
            from udwa.ingest import test_api_connection
    ...
    def list_metrics(self, connection_id: str) -> list[dict[str, Any]]:
        connection = self._connection(connection_id)
        try:
            from udwa.ingest import get_metrics_list
    ...
    def search_cages(self, connection_id: str, patterns: list[str]) -> list[dict[str, Any]]:
        ...
        connection = self._connection(connection_id)
        try:
            from udwa.ingest import search_cages_list
    ...
    def import_dataset(self, request: DVCImportRequest) -> DVCDatasetResult:
        ...
        try:
            from udwa.ingest import fetch_api_bundle
    ...
    def _persist(
        ...
        *,
        ...
        if timestamp_column and not measurements.empty:
            parsed = pd.to_datetime(measurements[timestamp_column], errors="coerce", utc=True).dropna()
            if not parsed.empty:
