...
class DVCApprovalNotFound(RuntimeError):
...
class FileDVCApprovalStore:
    ...
    def resolve(self, approval_id: str) -> DVCAnalysisApproval:
        if not re.fullmatch(r"[A-Za-z0-9._-]{1,200}", approval_id):
            raise DVCApprovalNotFound("Invalid DVC approval id.")
        path = (self.root / f"{approval_id}.json").resolve()
        root = self.root.resolve()
        if root not in path.parents or not path.is_file():
            raise DVCApprovalNotFound(f"DVC approval not found: {approval_id}")
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            approval = DVCAnalysisApproval.model_validate(payload)
        except Exception as exc:  # noqa: BLE001
            raise DVCApprovalNotFound(f"DVC approval record is invalid: {approval_id}") from exc
        if approval.approval_id != approval_id:
            raise DVCApprovalNotFound("DVC approval id does not match its record.")
        return approval
