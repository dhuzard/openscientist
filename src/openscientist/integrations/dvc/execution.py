...
class DVCAnalysisAsset(StrictModel):
    role: Literal["result", "provenance"]
    relative_path: str
    sha256: str
...
class DVCAnalysisBlocked(RuntimeError):
...
def _asset(job_dir: Path, path: Path, role: str) -> DVCAnalysisAsset:
...
class DVCAnalysisService:
    ...
    def execute(self, request: DVCAnalysisRequest) -> DVCAnalysisResult:
        ...
        if blockers:
            raise DVCAnalysisBlocked("DVC analysis is blocked.", blockers=blockers)
        ...
        try:
            from udwa.orchestrator import run_tool

            output = run_tool(request.operation, dataframe, **request.parameters)
        except Exception as exc:  # noqa: BLE001
            raise DVCAnalysisError(f"UDWA execution failed: {exc}") from exc
        if output.get("error"):
