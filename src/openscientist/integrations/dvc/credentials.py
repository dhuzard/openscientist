...
class DVCConnectionNotFoundError(LookupError):
...
def _environment_prefix(connection_id: str) -> str:
    ...
    if not normalized:
        raise DVCConnectionNotFoundError("DVC connection id must contain letters or numbers.")
...
class EnvironmentDVCConnectionProvider:
    ...
    def resolve(self, connection_id: str) -> DVCConnection:
        ...
        if not connection_id:
            raise DVCConnectionNotFoundError("DVC connection id is required.")
