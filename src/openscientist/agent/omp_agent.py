...
class OmpAgent(AbstractAgent[Provider]):
    ...
    async def _run_omp(self, prompt: str) -> IterationResult:
        ...
        # Windows does not honor shebangs via CreateProcess. This path is mainly
        # useful for local tests and developer-supplied Python wrappers; the
        # production agent image executes omp directly on Linux.
        if os.name == "nt":
