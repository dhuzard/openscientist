...
_POSIX_SHELL_ONLY = pytest.mark.skipif(
    os.name == "nt",
    reason="entrypoint rendering requires a host POSIX shell; Docker behaviour tests still run",
)
...
class _Lab:
    ...
    def run_probe(self, allow: str, source: str, target: str) -> dict[str, Any]:
        container = self._client.containers.run(
            TEST_IMAGE,
            # Normalize the bind-mounted working-tree copy before execution.
            # Git's index is LF (enforced by .gitattributes), but Docker Desktop
            # may expose a pre-existing Windows checkout with CRLF here.
            entrypoint=["/bin/sh"],
            command=[
                "-c",
                "sed 's/\\r$//' /source-entrypoint.sh > /tmp/entrypoint.sh && "
                'exec /bin/sh /tmp/entrypoint.sh "$@"',
                "firewall-test",
                "python3",
                "-c",
                source,
